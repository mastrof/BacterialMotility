...
run!, turn! and sense! can, in principle, all depend AND act in-place on state.
...
If run!, turn!, sense! are not defined, they are assigned to the identity function.
"""
@with_kw struct Bacterium{D,T,F,S} <: AbstractBacterium{D,T,F,S}
    ...
    run!::F = identity# displacement
    turn!::F = identity# reorientation
    sense!::F = identity# kinetic/tactic behavior
    state::S = InternalState() # internal state variables
end # state
