...
N = 20
pop1 = [Bacterium{d}(
    id = species[1], r = randposition(), v = randvelocity(40.0),
    run! = run!, turn! = tumble!, state = copy(s1)) for _ in 1:N]
pop2 = [Bacterium{d}(
    id = species[2], r = randposition(), v = randvelocity(25.0),
    run! = run!, turn! = custom_tumble!, state = copy(s2)) for _ in 1:N]
pop3 = [Bacterium{d}(
    id = species[3], r = randposition(), v = randvelocity(36.0),
...
global_callback(bs; kwargs...) = save_traj!(trajectories, bs)

bs = BacterialSystem(callback_inner = (b,f) -> rotational_diffusion!(b),
                     callback_outer = global_callback,
...
specieslabels = ["Small Δt", "Intermediate Δt", "Large Δt"]
